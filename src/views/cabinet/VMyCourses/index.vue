...
<!--      <a href="#" class="back-link" @click.prevent="$router.go(-1)">← Назад</a>-->
    ...
    <div class="col col-8">
      ...
      <div class="my-course-list">
        <v-course-card
            v-for="(item, itemIdx) in myCourseList"
            :key="itemIdx"
        ...
        />
<!--        <a href="#" class="my-course-link">+ Добавить курс</a>-->
      </div>
<!--      <br>-->
<!--      <br>-->
<!--      <b class="title-xs">Рекомендации</b>-->
<!--      <div class="my-course-list">-->
<!--        <div-->
<!--            class="recommend-course"-->
<!--            v-for="(item, itemIdx) in courseList"-->
<!--            :key="itemIdx"-->
<!--        >-->
<!--          <p>Курс «{{item.name}}»</p>-->
<!--          <v-button type="btn-danger" @click="openApplicationModal(item.id)">Перейти к покупке</v-button>-->
<!--        </div>-->
<!--      </div>-->
    </div>
...
export default {
  ...
  methods: {
    fetchMyCourses() {
      this.API.course.getClientCourseList()
          .then((res) => {
            this.myCourseList = res?.data.group_courses.concat(res?.data.personal_courses)
          })
    },
    fetchCourseList() {
    ...
    openApplicationModal(id) {
      this.vGlModal.show({
        modalName: 'v-application-modal',
        modalWidth: 320,
        component: 'v-application-modal',
        componentProps: {
          courseId: Number(id),
        }
      })
