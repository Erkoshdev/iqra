...
<!--        <a href="#" class="back-link" @click.prevent="$router.go(-1)">← Назад</a>-->
        <v-button type="btn-info-light-outline btn-add btn-square" @click="$router.push({name: 'VAdminUserAdd'})">+ Новый пользователь</v-button>
      </div>
      <div class="col col-8">
        <b class="title-xs">Все пользователи</b>
...
export default {
  ...
  methods: {
    ...
    handleEdit(id) {
      this.$router.push({name: 'VAdminUserEdit', params: {id: id}})
    },
      handleDelete(id) {
        this.vGlModal.show({
