    ...
    handleSaveList() {
      if (this.changeStatus.length > 0) {
        this.changeStatus.forEach(user => {
          ...
          this.API.course.changeAccess(this.courseId, body)
              .then(() => {
                this.$router.push({name: 'VAdminCourseStudents', params: {id: this.courseId}})
              })
        })
      } else this.$router.push({name: 'VAdminCourseStudents', params: {id: this.courseId}})
